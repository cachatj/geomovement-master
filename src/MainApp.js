...
class MainApp extends React.Component {
  constructor(props) {
    ...
    this.layers = {
      1: {
        name: "bins1",
        url: "mapbox://scottpez.ckgowj5ev0sk22bs6m3iwae4f-43d05",
      },
      2: {
        name: "bins2",
        url: "mapbox://scottpez.ckgowjxnx0jry27oxsch1epd3-2g530",
      },
      3: {
        name: "states",
        url: "mapbox://scottpez.ckgowlzft1i8x23mnky4ecv33-2vi4o",
      },
      4: {
        name: "countries",
        url: "mapbox://scottpez.ckgowoh3z1l1c22qik5l28lp0-1opjt",
      },
      5: {
        name: "continents",
        url: "mapbox://scottpez.ckgoz4ol50o0122mn5ddci5vg-5ojyo",
      },
    };
    this.selectedLayer = 1;
    this.previousLayer = 1;
    this.facadeBaseUrl = this.production
      ? "/facade/"
      : "http://localhost:8000/facade/";
    this.elasticGeostatementRequest = "geostatement";
    this.elasticGeobinRequest = "geobin";
    this.elasticSuggestRequest = "suggest";
    this.geomovementLabelUpdateUrl =
      "http://localhost:8081/PezResearchWeb/GeomovementLabelUpdate?";
    this.colorRamps = {
      blue: {
        1: "#c6dbef",
        2: "#9ecae1",
    ...
    this.mapStyles = {
      ...
      arcs: {
        "classes-line-color-source": {
          ...
          3: {
            1: [65, 171, 93, this.dAlpha2],
            2: [35, 139, 69, 0],
            3: [0, 90, 50, 0],
          },
          4: {
            1: [186, 228, 179, this.dAlpha2],
            2: [116, 196, 118, 0],
            3: [49, 163, 84, 0],
            4: [0, 109, 44, 0],
          },
          5: {
            1: [199, 233, 192, this.dAlpha2],
            2: [161, 217, 155, 0],
            3: [116, 196, 118, 0],
            4: [49, 163, 84, 0],
            5: [0, 109, 44, 0],
        ...
        "classes-line-color-target": {
          ...
          3: {
            1: [65, 171, 93, this.dAlpha],
            2: [35, 139, 69, this.dAlpha],
            3: [0, 90, 50, this.dAlpha],
          },
          4: {
            1: [186, 228, 179, this.dAlpha],
            2: [116, 196, 118, this.dAlpha],
            3: [49, 163, 84, this.dAlpha],
            4: [0, 109, 44, this.dAlpha],
          },
          5: {
            1: [199, 233, 192, this.dAlpha],
            2: [161, 217, 155, this.dAlpha],
            3: [116, 196, 118, this.dAlpha],
            4: [49, 163, 84, this.dAlpha],
            5: [0, 109, 44, this.dAlpha],
          },
          6: {
            1: [199, 233, 192, this.dAlpha],
            2: [161, 217, 155, this.dAlpha],
            3: [116, 196, 118, this.dAlpha],
            4: [65, 171, 93, this.dAlpha],
            5: [35, 139, 69, this.dAlpha],
            6: [0, 90, 50, this.dAlpha],
          },
          7: {
            1: [229, 245, 224, this.dAlpha],
            2: [199, 233, 192, this.dAlpha],
            3: [161, 217, 155, this.dAlpha],
            4: [116, 196, 118, this.dAlpha],
            5: [65, 171, 93, this.dAlpha],
            6: [35, 139, 69, this.dAlpha],
            7: [0, 90, 50, this.dAlpha],
          },
        },
        "classes-line-width": {
          1: { 1: 3 },
    ...
    if (features.length > 0) {
      ...
      for (let i = 0; i < fkeys.length; i++) {
        ...
        if (features[fkeys[i]].properties.n) {
          this.mylocationsNames[parseInt(features[fkeys[i]].properties.id)] =
            features[fkeys[i]].properties.n;
        } else {
          this.mylocationsNames[parseInt(features[fkeys[i]].properties.id)] =
            features[fkeys[i]].properties.id;
        }
  ...
  getNewFeatures = (point, featuresType) => {
    let features = [];
    if (point) {
      let fs = this.map.queryRenderedFeatures(point, {
        layers: [this.layers[this.selectedLayer].name],
      });
    ...
    if (featuresType === "newScale") {
      for (let i = 0; i < this.state.clickLngLats.length; i++) {
        ...
        let fs = this.map.queryRenderedFeatures(mypoint, {
          layers: [this.layers[this.selectedLayer].name],
        });
        for (let j = 0; j < fs.length; j++) {
          features.push(fs[j]);
  ...
  sendRequests = (point, lngLat, featuresType, newFeatures, newParams) => {
    if (newFeatures.length > 0 || featuresType === "newScale") {
      let params = this.getBinIdsFromFeatures(
        point,
        lngLat,
        featuresType,
        newFeatures,
  ...
  retrieveStatements = (newParams, e) => {
    ...
    if (newFeatures.length > 0) {
      ...
      if (this.map.getLayer("highlightLabels1")) {
        this.map.removeLayer("highlightLabels1");
      }
      if (this.map.getLayer("highlightLabels2")) {
        this.map.removeLayer("highlightLabels2");
      }
    }
  };
    ...
    if (numClasses > 0) {
      ...
      switch (type) {
        ...
        case "stdDeviation":
          ...
          classBreaks = classBreaks.slice(
            classBreaks.length - numClasses,
            classBreaks.length,
          );
  ...
  drawConnectionsLayer = (connections, newBinClasses) => {
    if (!connections) {
      if (this.map.getLayer("arcs")) {
        ...
        if (connArr.length > 0) {
          ...
          for (let i = 0; i < connArr.length; i++) {
            ...
            let sColor =
              this.mapStyles.arcs["classes-line-color-source"][
                breakers.length - 1
              ][myClass2];
            let tColor =
              this.mapStyles.arcs["classes-line-color-target"][
                breakers.length - 1
              ][myClass2];
            connectionsData.push({
              ...
              myClass: myClass2,
              width:
                this.mapStyles.arcs["classes-line-width"][breakers.length - 1][
                  myClass2
                ],
            });
          }
      ...
      for (let i = 0; i < connKeys.length; i++) {
        ...
        connObj[sourceId].push({
          source: sourceId,
          target: targetId,
          count: connections[connKeys[i]],
        });
      }
      ...
      if (ks.length > 0) {
        ...
        for (let j = 0; j < ks.length; j++) {
          ...
          for (let i = 0; i < cs.length; i++) {
            ...
            let sourceCoords =
              geogCentroids[this.layers[this.selectedLayer].name][sourceId];
            let targetCoords =
              geogCentroids[this.layers[this.selectedLayer].name][targetId];
            if (sourceCoords && targetCoords) {
              connectionsData.push({
                ...
                myClass: myClass2,
                width:
                  this.mapStyles.arcs["classes-line-width"][
                    breakers.length - 1
                  ][myClass2],
              });
            } else {
  ...
  drawStatementsLayers = () => {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      for (let j = 0; j < statements.length; j++) {
        ...
        if (!this.production) {
          this.statementButtonStyles[statements[j].statementId + "_y"] =
            this.props.classes.notChosen;
          this.statementButtonStyles[statements[j].statementId + "_x"] =
            this.props.classes.notChosen;
        }
        allids.push(binId);
  ...
  setStatements = (myStatements) => {
    ...
    for (let i = 0; i < myStatements.length; i++) {
      ...
      let items = myCurrentStatements.filter(
        (statement2) => statement2.statementId === statement1.statementId,
      );
      if (items.length === 0) {
  ...
  buildRetrieveStatementsRequest = (newParams) => {
    let requestParams = {
      ...
      i: newParams.channels
        ? newParams.channels.join()
  ...
  removeBigram = (id, binName) => {
    ...
    bigrams[binName].splice(
      bigrams[binName].findIndex((item) => item.id === id),
      1,
    );
    this.setState({
      bigramsTableRows: bigrams,
      bigramUpdate: !this.state.bigramUpdate,
    });
  };
  ...
  updateBins = (lData, newBinClasses) => {
    ...
    for (let i = 0; i < bins.length; i++) {
      ...
      ids.push(binKey);
      binIdColors.push(
        this.colorRamps.maroon[classBreaks.length - 1][myClass + 1],
      );
    }
    ...
    this.mapStyles.bins["fill-color"] = [
      "match",
      ["get", "id"],
      ...binIdColors,
      "#000000",
    ...
    this.map.setPaintProperty(
      this.layers[this.selectedLayer].name,
      "fill-color",
      this.mapStyles.bins["fill-color"],
    );
  };
  ...
  getLargestFeatures = (features) => {
    ...
    for (let i = 0; i < features.length; i++) {
      ...
      if (
        !returnFeatures[features[i].properties.id] ||
        newArea > returnFeatures[features[i].properties.id].properties.area
      ) {
  ...
  handleHighlightLabels = (ids) => {
    ...
    if (this.layers[this.selectedLayer].name.includes("bins")) {
      ...
      let features = this.map.querySourceFeatures(
        this.layers[this.selectedLayer].name,
        { sourceLayer: this.layers[this.selectedLayer].name, filter: filter },
      );
  ...
  removePreviousLayer = () => {
    ...
    if (this.map.getLayer(this.previousLayerName)) {
      this.map.removeLayer(this.previousLayerName);
    }
    if (this.map.getSource(this.previousLayerName)) {
      this.map.removeSource(this.previousLayerName);
    }
    if (this.map.getLayer(this.layers[this.selectedLayer].name)) {
      this.map.removeLayer(this.layers[this.selectedLayer].name);
    }
    if (this.map.getSource(this.layers[this.selectedLayer].name)) {
      this.map.removeSource(this.layers[this.selectedLayer].name);
    }
    if (this.map.getLayer("arcs")) this.map.removeLayer("arcs");
  };
  handleLayers = (params) => {
  ...
  handleControls = () => {
    ...
    this.map.on("zoomend", () => {
      ...
      if (
        layerName.includes("bins") &&
        layerName.slice(-1) !== this.getBinVersion()
      ) {
        ...
        renderCell: (params) => (
          <div>
            <IconButton
              size={"small"}
              onClick={(e) => {
                this.handleMovementClick(e, params.value, "y");
              }}
            ...
            <IconButton
              size={"small"}
              onClick={(e) => {
                this.handleMovementClick(e, params.value, "x");
              }}
